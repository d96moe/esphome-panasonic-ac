...
void PanasonicACCNT::check_for_anomaly() {
  ...
  // checked. 0x80/0xC0/0xC1 are the three slots documented at
  // https://github.com/ssjoholm/panasonic-cn-cnt (tested on CS-HZ25/35XKE,
  // CS-HZ35ZKE). 0x00 is added on top - confirmed via a live 22s/5-packet
  // capture on this unit (a different model) that it's held perfectly stable
  // at 00 00 00 across packets where every other byte still moved normally,
  // so it's this model's own equivalent of one of the slots, just a value
  // the documented units don't use - not noise.
  bool known_b31 = b31 == 0x00 || b31 == 0x80 || b31 == 0xC0 || b31 == 0xC1;
